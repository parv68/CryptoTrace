...
fn main() {
    ...
    let high = threats
        .iter()
        .filter(|t| t.risk_score > 4.0 && t.risk_score <= 7.0)
        .count();
    let medium = threats
        .iter()
        .filter(|t| t.risk_score > 2.0 && t.risk_score <= 4.0)
        .count();
    ...
    println!(
        "{:6} {:50} {:12} {:8} {:20}",
        "Score", "Path", "Algorithm", "Entropy", "CVEs"
    );
    println!("{}", "-".repeat(100));

    for t in threats.iter().filter(|t| t.risk_score > 2.0).take(20) {
        let algo = t.algorithm.as_deref().unwrap_or("-");
        let cve_str = if t.cves.is_empty() {
            "-".into()
        } else {
            t.cves.join(",")
        };
        let path_trunc = if t.path.len() > 50 {
            format!("...{}", &t.path[t.path.len() - 47..])
        } else {
            t.path.clone()
        };
        println!(
            "{:>5.1}  {:50} {:12} {:>7.2} {:20}",
            t.risk_score, path_trunc, algo, t.entropy, cve_str
        );
    }

    let avg_entropy: f64 =
        threats.iter().map(|t| t.entropy).sum::<f64>() / threats.len().max(1) as f64;
    println!("\n=== Statistics ===");
    println!("  Average entropy: {:.2}", avg_entropy);
    println!(
        "  Files with CVEs: {}",
        threats.iter().filter(|t| !t.cves.is_empty()).count()
    );
}
...
fn scan_directory(dir: &str, max_size: u64, threats: &mut Vec<ThreatEntry>) {
    ...
    for entry in iter.flatten() {
        ...
        if meta.len() > max_size || meta.len() < 4 {
            continue;
        }
