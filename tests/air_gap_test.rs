...
use cryptotrace::types::{AiCacheConfig, AiConfig, AppConfig, SourceType};
...
fn test_ai_disabled_by_default() {
    ...
    assert!(
        !ai.enabled,
        "AI must be disabled by default for air-gap compliance"
    );
    assert!(
        ai.base_url.is_none(),
        "base_url should be None when AI is disabled"
    );
}
...
fn test_analysis_completes_without_network() {
    ...
    assert!(
        !result.input_hash.is_empty(),
        "input hash should be populated"
    );
    assert!(
        !result.detected_type.is_empty(),
        "detected type should be populated"
    );
    assert!(result.confidence > 0.0, "confidence should be > 0");

    // AI narrative should be None (disabled by default)
    assert!(
        result.ai_narrative.is_none(),
        "AI narrative should be None when AI is disabled"
    );
...
fn test_all_network_features_opt_in() {
    ...
    assert!(
        config.ai.base_url.is_none(),
        "AI base URL should be None by default"
    );
    eprintln!(
